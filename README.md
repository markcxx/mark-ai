...
MarkAI 将正文、推理、工具调用、生成文件和翻译保存为带类型的消息片段。OpenAI-compatible 服务既可以返回结构化 reasoning，也可以在正文中使用 `<think>` 标签；服务端会在任意 SSE 分块边界上完成归一化，避免推理标签泄漏到最终回答。
...
后台的「模型管理」自动列出全部站点模型，可维护上新开关、结束时间、上新排序、
展示名称和一句话介绍。提供商与模型 ID 共同标识模型，用户自行配置的提供商不参与站点上新展示。

Web 新对话欢迎页会在输入框下方显示可用的上新模型，点击仅切换模型并保留草稿。
模型菜单同步显示上新标记和介绍；没有上新模型时不显示横栏。用户关闭一批上新后，
浏览器会记住选择，新增上新模型或重新发布已结束的上新才会再次展示。
结束时间按管理员所在时区输入，到期自动隐藏上新展示，模型仍可继续使用。

升级到此功能时先执行 `npm run db:migrate`，创建 `model_presentations` 表。
此功能不新增环境变量，不修改模型连接和密钥配置；后台修改会写入审计日志。
