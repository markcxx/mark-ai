...
export async function PATCH(req: NextRequest) {
  ...
  const siteConfig = getConfiguredModels().find(
    (item) =>
  ...
  if (!enabled) {
    await db
      ...
      .values({
        ...
        provider,
        runtime:
          existing?.runtime || template?.runtime || siteConfig?.runtime || "openai-compatible",
...
export async function PUT(req: NextRequest) {
  ...
  const name =
    typeof body?.name === "string" ? body.name.trim().slice(0, 60) : template?.name || provider;
  const runtime: ModelRuntime =
    template?.runtime || (body?.runtime === "gemini" ? "gemini" : "openai-compatible");
