...
export type PrimaryColor = 'blue' | 'cyan' | 'green' | 'indigo' | 'magenta' | 'orange' | 'red' | 'violet';
export type CodeTheme = 'dracula' | 'duotone' | 'github' | 'gruvbox' | 'material' | 'night-owl' | 'nord' | 'one' | 'solarized' | 'vscode';
...
export const sanitizeGeneralSettings = (
  ...
  return {
    ...
    codeColorMode: stringOption(input.codeColorMode, ['auto', 'light', 'dark'], fallback.codeColorMode),
    codeLineNumbers: booleanValue(input.codeLineNumbers, fallback.codeLineNumbers),
    codeTheme: stringOption(input.codeTheme, ['one', 'vscode', 'material', 'gruvbox', 'solarized', 'github', 'dracula', 'night-owl', 'nord', 'duotone'], fallback.codeTheme),
    ...
    primaryColor: stringOption(input.primaryColor, ['blue', 'indigo', 'violet', 'magenta', 'red', 'orange', 'green', 'cyan'], fallback.primaryColor),
    reduceMotion: booleanValue(input.reduceMotion, fallback.reduceMotion),
    responseAnimation: stringOption(input.responseAnimation, ['none', 'fade', 'smooth'], fallback.responseAnimation),
    ...
    thinkingDisplay: stringOption(input.thinkingDisplay, ['auto', 'collapsed', 'expanded'], fallback.thinkingDisplay),
...
export const sanitizeLanguageModelSettings = (
  ...
  return {
    ...
    personalizationEnabled: booleanValue(input.personalizationEnabled, fallback.personalizationEnabled),
    preferredName: shortText(input.preferredName, 60),
    responseLanguage: stringOption(input.responseLanguage, ['auto', 'zh-CN', 'en-US'], fallback.responseLanguage),
    responseLength: stringOption(input.responseLength, ['concise', 'balanced', 'detailed'], fallback.responseLength),
    responseTone: stringOption(input.responseTone, ['direct', 'friendly', 'professional'], fallback.responseTone),
  };
};
