...
const parseReaderMarkdown = (markdown: string, fallbackUrl: URL) => {
  ...
  const content = publishedIndex >= 0
  ...
  try {
    const response = await fetch(getReaderUrl(url), {
      headers: {
        ...
        'User-Agent':
          'Mozilla/5.0 (compatible; MarkAI-WebReader/1.0; +https://markai.local)',
      },
...
const readViaFirecrawl = async (url: URL, startedAt: number, signal?: AbortSignal) => {
  ...
  for (const apiKey of apiKeys) {
    ...
    try {
      ...
      const content = normalizeWhitespace(String(page.markdown || page.text || page.html || ''))
...
const readViaTavilyExtract = async (url: URL, startedAt: number, signal?: AbortSignal) => {
  ...
  for (const apiKey of apiKeys) {
    ...
    try {
      ...
      if (!response.ok) {
        lastError = await getProviderErrorMessage(response, `Tavily Extract HTTP ${response.status}`);
        continue;
      }
      ...
      const content = normalizeWhitespace(String(result?.raw_content || result?.content || ''))
        .slice(0, MAX_CONTENT_CHARS);
...
export async function readWebpage({
  ...
  for (let redirect = 0; redirect <= MAX_REDIRECTS; redirect += 1) {
    ...
    try {
      ...
      if (
        contentType &&
        !/text\/html|application\/xhtml\+xml|text\/plain|application\/xml|text\/xml/i.test(contentType)
      ) {
