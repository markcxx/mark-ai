...
export default function ChatApp({ initialSessionId }: { initialSessionId?: string }) {
  ...
  return (
    ...
    >
      <div className="flex h-dvh w-screen flex-col overflow-hidden bg-[var(--chat-app-bg)]">
        <AnnouncementBar />
        <div
          className={cn(
            "relative flex min-h-0 w-full flex-1 overflow-clip bg-[var(--chat-app-bg)] p-0 font-sans text-gray-900 antialiased dark:text-gray-100 md:p-2",
        ...
        >
          ...
          {isMobileViewport && isSidebarOpen && (
            <button
              aria-label="收起历史会话"
              className="absolute inset-y-0 right-0 z-30 bg-transparent"
                ...
                {isLoadingActiveSession ? (
                  ...
                  <WelcomePanel>
                    <ChatInput
                      ...
                      setModelSearchKeyword={(kw) =>
                        useUIStore.getState().setModelSearchKeyword(kw)
                      }
