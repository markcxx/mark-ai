...
export function MessageSelectionWrapper({
  ...
  return (
    <button
      className={cn(
        'group/selection -mx-4 flex w-[calc(100%+32px)] animate-in fade-in slide-in-from-left-1 items-center gap-3 px-4 py-2 text-left transition-[background-color,transform] duration-150 ease-out hover:bg-gray-100/70 active:scale-[0.998] md:-mx-8 md:w-[calc(100%+64px)] md:px-8',
        isSelected && 'bg-gray-100/80 hover:bg-gray-100/80',
      )}
      onClick={handleClick}
      type="button"
    >
      <span
        className={cn(
          ...
          isSelected
            ...
            : 'scale-95 border-gray-300 bg-white text-transparent group-hover/selection:scale-100 group-hover/selection:border-gray-500',
        )}
      >
        <Check
          className={cn(
