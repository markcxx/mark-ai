// Generated from lib/model-metadata.ts and lib/chat/image-models.ts.
const modelMetadata = [
  {
    "aliases": ["gpt-6-astra-低", "gpt-6-astra-中", "gpt-6-astra-高"],
    "contextWindowTokens": 1050000,
    "description": "OpenAI 旗舰推理模型，面向复杂推理、编码、计算机使用、研究和文档创作。",
    "displayName": "GPT-6 Astra",
    "family": "GPT-6",
    "id": "gpt-6-astra",
    "knowledgeCutoff": "2026-04-30",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-6-astra",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVision": true,
  },
  {
    "description": "GPT-6 配置标识；官方已公布 GPT-6 Astra，尚未核实此独立标识的规格，请以服务商配置为准。",
    "displayName": "GPT-6",
    "family": "GPT-6",
    "id": "gpt-6",
    "sourceUrl": "https://developers.openai.com/api/docs/models",
  },
  {
    "aliases": ["gpt-5.6-sol-低", "gpt-5.6-sol-中", "gpt-5.6-sol-高"],
    "contextWindowTokens": 1050000,
    "description": "OpenAI GPT-5.6 系列旗舰推理模型，面向复杂专业工作、编码、研究和高级 Agent 工作流。",
    "displayName": "GPT-5.6 Sol",
    "family": "GPT-5.6",
    "id": "gpt-5.6-sol",
    "knowledgeCutoff": "2026-02-16",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-5.6-sol",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVision": true,
  },
  {
    "aliases": ["gpt-5.6-terra-低", "gpt-5.6-terra-中", "gpt-5.6-terra-高"],
    "contextWindowTokens": 1050000,
    "description": "OpenAI GPT-5.6 系列均衡型号，在智能水平、速度和成本之间取得平衡。",
    "displayName": "GPT-5.6 Terra",
    "family": "GPT-5.6",
    "id": "gpt-5.6-terra",
    "knowledgeCutoff": "2026-02-16",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-5.6-terra",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVision": true,
  },
  {
    "aliases": ["gpt-5.6-luna-低", "gpt-5.6-luna-中", "gpt-5.6-luna-高"],
    "contextWindowTokens": 1050000,
    "description": "OpenAI GPT-5.6 系列高效型号，针对成本敏感、高吞吐和轻量工作负载优化。",
    "displayName": "GPT-5.6 Luna",
    "family": "GPT-5.6",
    "id": "gpt-5.6-luna",
    "knowledgeCutoff": "2026-02-16",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-5.6-luna",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVision": true,
  },
  {
    "aliases": ["gpt-5-5"],
    "contextWindowTokens": 1050000,
    "description": "OpenAI 面向复杂专业工作、编码和长程 Agent 任务的前沿推理模型。",
    "displayName": "GPT-5.5",
    "family": "GPT-5",
    "id": "gpt-5.5",
    "knowledgeCutoff": "2025-12",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-5.5",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVision": true,
  },
  {
    "aliases": ["gpt-5-3"],
    "contextWindowTokens": 128000,
    "description": "ChatGPT 使用的 GPT-5.3 Instant 对话模型，支持图片输入和函数调用。",
    "displayName": "GPT-5.3 Chat",
    "family": "GPT-5",
    "id": "gpt-5.3-chat-latest",
    "knowledgeCutoff": "2025-08",
    "maxOutputTokens": 16384,
    "sourceUrl":
        "https://developers.openai.com/api/docs/models/gpt-5.3-chat-latest",
    "supportsToolCalling": true,
    "supportsVision": true,
  },
  {
    "description": "ChatGPT 和部分代理服务使用的内部轻量路由标识，公开资料未披露可靠的上下文与能力规格。",
    "displayName": "GPT-5.3 Mini",
    "family": "GPT-5",
    "id": "gpt-5-3-mini",
    "sourceUrl": "https://community.openai.com/t/gpt-5-5-go-redirects-to-gpt-5-3-mini-for-more-than-24-hours/1385556",
  },
  {
    "aliases": ["gpt-5.4-pro"],
    "contextWindowTokens": 1050000,
    "description": "面向复杂专业工作与高难度推理的 OpenAI 前沿模型。",
    "displayName": "GPT-5.4",
    "family": "GPT-5",
    "id": "gpt-5.4",
    "knowledgeCutoff": "2025-08",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-5.4",
  },
  {
    "contextWindowTokens": 400000,
    "description": "兼顾成本与能力的 GPT-5.4 轻量型号，适合编码与高频任务。",
    "displayName": "GPT-5.4 mini",
    "family": "GPT-5",
    "id": "gpt-5.4-mini",
    "knowledgeCutoff": "2025-08",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models",
  },
  {
    "contextWindowTokens": 400000,
    "description": "面向简单、高并发任务的低成本 GPT-5.4 型号。",
    "displayName": "GPT-5.4 nano",
    "family": "GPT-5",
    "id": "gpt-5.4-nano",
    "knowledgeCutoff": "2025-08",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models",
  },
  {
    "aliases": ["gpt-5.2-codex"],
    "contextWindowTokens": 400000,
    "description": "擅长编码、代理工作流和长上下文推理的旗舰模型。",
    "displayName": "GPT-5.2",
    "family": "GPT-5",
    "id": "gpt-5.2",
    "knowledgeCutoff": "2025-08",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-5.2",
  },
  {
    "aliases": ["gpt-5-mini", "gpt-5-nano"],
    "contextWindowTokens": 400000,
    "description": "适合跨领域编码、推理和工具调用的通用 GPT-5 模型。",
    "displayName": "GPT-5",
    "family": "GPT-5",
    "id": "gpt-5",
    "knowledgeCutoff": "2024-09",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-5",
  },
  {
    "aliases": ["gpt-4.1-mini", "gpt-4.1-nano"],
    "contextWindowTokens": 1047576,
    "description": "具备百万级上下文，适合长文档、代码库和复杂指令处理。",
    "displayName": "GPT-4.1",
    "family": "GPT-4.1",
    "id": "gpt-4.1",
    "knowledgeCutoff": "2024-06",
    "maxOutputTokens": 32768,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-4.1",
  },
  {
    "contextWindowTokens": 200000,
    "description": "OpenAI 小型推理模型，适合数学、编码和视觉推理。",
    "displayName": "o4-mini",
    "family": "OpenAI o-series",
    "id": "o4-mini",
    "maxOutputTokens": 100000,
    "sourceUrl": "https://developers.openai.com/api/docs/models/o4-mini",
  },
  {
    "contextWindowTokens": 1114112,
    "description": "Google 面向速度优化的高级多模态推理模型，支持搜索与 grounding。",
    "displayName": "Gemini 3.5 Flash",
    "family": "Gemini 3.5",
    "id": "gemini-3.5-flash",
    "knowledgeCutoff": "2025-01",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://ai.google.dev/gemini-api/docs/models",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 1048576,
    "description": "Google 面向真实 Agent 工作负载的高速前沿模型，擅长代码生成、代理执行和空间推理。",
    "displayName": "Gemini 3.6 Flash",
    "family": "Gemini 3.6",
    "id": "gemini-3.6-flash",
    "maxOutputTokens": 65536,
    "sourceUrl":
        "https://ai.google.dev/gemini-api/docs/models/gemini-3.6-flash",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 1048576,
    "description": "Google 面向子 Agent、文档解析和高吞吐工作流的低延迟、低成本多模态模型。",
    "displayName": "Gemini 3.5 Flash-Lite",
    "family": "Gemini 3.5",
    "id": "gemini-3.5-flash-lite",
    "maxOutputTokens": 65536,
    "sourceUrl":
        "https://ai.google.dev/gemini-api/docs/models/gemini-3.5-flash-lite",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": ["gemini-3.1-pro-preview", "gemini-pro-latest"],
    "contextWindowTokens": 1114112,
    "description": "Gemini 3.1 Pro 在 Gemini 3 Pro 基础上增强复杂推理，并支持更细粒度的思考强度控制。",
    "displayName": "Gemini 3.1 Pro",
    "family": "Gemini 3.1",
    "id": "gemini-3.1-pro",
    "knowledgeCutoff": "2025-01",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://ai.google.dev/gemini-api/docs/models",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": ["gemini-3.1-flash-lite-preview", "gemini-flash-lite-latest"],
    "contextWindowTokens": 1114112,
    "description": "Google 面向高吞吐代理任务、翻译和数据处理的高性价比多模态模型。",
    "displayName": "Gemini 3.1 Flash-Lite",
    "family": "Gemini 3.1",
    "id": "gemini-3.1-flash-lite",
    "knowledgeCutoff": "2025-01",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://ai.google.dev/gemini-api/docs/models",
  },
  {
    "aliases": ["gemini-3-flash", "gemini-flash-latest"],
    "contextWindowTokens": 1114112,
    "description": "兼顾前沿推理、生成速度与搜索 grounding 的 Gemini 3 快速型号。",
    "displayName": "Gemini 3 Flash",
    "family": "Gemini 3",
    "id": "gemini-3-flash-preview",
    "knowledgeCutoff": "2025-01",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://ai.google.dev/gemini-api/docs/models",
  },
  {
    "contextWindowTokens": 1048576,
    "description": "Google 高级推理模型，适合复杂代码、数学和长文档分析。",
    "displayName": "Gemini 2.5 Pro",
    "family": "Gemini 2.5",
    "id": "gemini-2.5-pro",
    "knowledgeCutoff": "2025-01",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://ai.google.dev/gemini-api/docs/models",
  },
  {
    "aliases": ["gemini-2.5-flash-lite"],
    "contextWindowTokens": 1048576,
    "description": "兼顾速度、成本和推理能力的 Google 多模态模型。",
    "displayName": "Gemini 2.5 Flash",
    "family": "Gemini 2.5",
    "id": "gemini-2.5-flash",
    "knowledgeCutoff": "2025-01",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://ai.google.dev/gemini-api/docs/models",
  },
  {
    "contextWindowTokens": 1048576,
    "description": "DeepSeek 原生多模态模型，支持图像理解、复杂推理、编码和工具调用。",
    "displayName": "DeepSeek V4.1 Flash",
    "family": "DeepSeek V4.1",
    "id": "deepseek-v4.1-flash",
    "maxOutputTokens": 393216,
    "sourceUrl": "https://huggingface.co/deepseek-ai/DeepSeek-V4.1-Flash",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVision": true,
  },
  {
    "aliases": ["deepseek-v4-pro-260425"],
    "contextWindowTokens": 1048576,
    "description": "DeepSeek V4 系列旗舰模型，支持百万级上下文、多档推理强度和代理任务。",
    "displayName": "DeepSeek V4 Pro",
    "family": "DeepSeek V4",
    "id": "deepseek-v4-pro",
    "maxOutputTokens": 393216,
    "sourceUrl": "https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "aliases": ["deepseek-v4-flash-260425"],
    "contextWindowTokens": 1048576,
    "description": "DeepSeek V4 系列高效预览模型，支持百万级上下文、推理和代理任务。",
    "displayName": "DeepSeek V4 Flash",
    "family": "DeepSeek V4",
    "id": "deepseek-v4-flash",
    "maxOutputTokens": 393216,
    "sourceUrl": "https://huggingface.co/deepseek-ai/DeepSeek-V4-Flash",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "aliases": ["deepseek-reasoner"],
    "contextWindowTokens": 131072,
    "description": "DeepSeek 通用对话与推理模型，支持思考和工具调用场景。",
    "displayName": "DeepSeek Chat",
    "family": "DeepSeek",
    "id": "deepseek-chat",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://api-docs.deepseek.com/quick_start/pricing",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 163840,
    "description": "DeepSeek 混合推理与 Agent 模型，强化长上下文、推理和工具使用。",
    "displayName": "DeepSeek V3.2",
    "family": "DeepSeek V3",
    "id": "deepseek-v3.2",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://huggingface.co/deepseek-ai/DeepSeek-V3.2",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 131072,
    "description": "DeepSeek V3 开源 MoE 对话模型，适合通用问答、代码和长文本任务。",
    "displayName": "DeepSeek V3",
    "family": "DeepSeek V3",
    "id": "deepseek-v3",
    "sourceUrl": "https://huggingface.co/deepseek-ai/DeepSeek-V3",
  },
  {
    "contextWindowTokens": 131072,
    "description": "DeepSeek 开源推理模型，擅长数学、编程和复杂逻辑任务。",
    "displayName": "DeepSeek R1",
    "family": "DeepSeek R1",
    "id": "deepseek-r1",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://huggingface.co/deepseek-ai/DeepSeek-R1",
    "supportsReasoning": true,
  },
  {
    "aliases": ["qwen3.7-max-preview"],
    "contextWindowTokens": 1000000,
    "description": "通义千问旗舰 Agent 模型，支持图像、视频理解、推理和函数调用。",
    "displayName": "Qwen 3.7 Max",
    "family": "Qwen 3.7",
    "id": "qwen3.7-max",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://help.aliyun.com/zh/model-studio/models",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "通义千问多模态交互 Agent 模型，支持视觉、视频、推理和函数调用。",
    "displayName": "Qwen 3.7 Plus",
    "family": "Qwen 3.7",
    "id": "qwen3.7-plus",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://help.aliyun.com/zh/model-studio/models",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 262144,
    "description": "通义千问 3.6 系列文本旗舰预览模型，支持推理与函数调用。",
    "displayName": "Qwen 3.6 Max Preview",
    "family": "Qwen 3.6",
    "id": "qwen3.6-max-preview",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://help.aliyun.com/zh/model-studio/models",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "通义千问多模态通用模型，支持文本、图像、视频、推理和函数调用。",
    "displayName": "Qwen 3.6 Plus",
    "family": "Qwen 3.6",
    "id": "qwen3.6-plus",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://help.aliyun.com/zh/model-studio/models",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "通义千问低延迟多模态模型，支持图像、视频、推理和函数调用。",
    "displayName": "Qwen 3.6 Flash",
    "family": "Qwen 3.6",
    "id": "qwen3.6-flash",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://help.aliyun.com/zh/model-studio/models",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 262144,
    "description": "Qwen 3.6 稀疏 MoE 多模态模型，支持图像理解、长上下文和 Agentic Coding。",
    "displayName": "Qwen 3.6 35B A3B",
    "family": "Qwen 3.6",
    "id": "qwen3.6-35b-a3b",
    "maxOutputTokens": 81920,
    "sourceUrl": "https://huggingface.co/Qwen/Qwen3.6-35B-A3B",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 262144,
    "description": "Qwen 3.6 稠密多模态模型，支持长上下文、视觉理解、推理和工具调用。",
    "displayName": "Qwen 3.6 27B",
    "family": "Qwen 3.6",
    "id": "qwen3.6-27b",
    "maxOutputTokens": 81920,
    "sourceUrl": "https://huggingface.co/Qwen/Qwen3.6-27B",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "通义千问高性价比多模态模型，支持图像、视频、推理和函数调用。",
    "displayName": "Qwen 3.5 Flash",
    "family": "Qwen 3.5",
    "id": "qwen3.5-flash",
    "maxOutputTokens": 65536,
    "sourceUrl":
        "https://help.aliyun.com/zh/model-studio/text-generation-model/",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 262144,
    "description": "Qwen 3.5 大规模 MoE 多模态模型，支持文本与图像输入。",
    "displayName": "Qwen 3.5 397B A17B",
    "family": "Qwen 3.5",
    "id": "qwen3.5-397b-a17b",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://huggingface.co/Qwen/Qwen3.5-397B-A17B",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 262144,
    "description": "Qwen3-VL 轻量视觉模型，支持长视频、视觉推理和函数调用。",
    "displayName": "Qwen 3 VL Flash",
    "family": "Qwen 3 VL",
    "id": "qwen3-vl-flash",
    "maxOutputTokens": 32768,
    "sourceUrl": "https://help.aliyun.com/zh/model-studio/vision-model/",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 131072,
    "description": "Qwen 3 非思考指令模型，强化多语言指令遵循并支持函数调用。",
    "displayName": "Qwen 3 235B A22B Instruct 2507",
    "family": "Qwen 3",
    "id": "qwen3-235b-a22b-instruct-2507",
    "maxOutputTokens": 32768,
    "sourceUrl": "https://huggingface.co/Qwen/Qwen3-235B-A22B-Instruct-2507",
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 131072,
    "description": "Qwen 3 稠密参数模型，支持思考与非思考模式、函数调用和结构化输出。",
    "displayName": "Qwen 3 32B",
    "family": "Qwen 3",
    "id": "qwen3-32b",
    "maxOutputTokens": 32768,
    "sourceUrl":
        "https://help.aliyun.com/zh/model-studio/text-generation-model/",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "通义千问长上下文代码模型，面向代码生成、工程任务和 Agent 工作流。",
    "displayName": "Qwen 3 Coder Plus",
    "family": "Qwen 3 Coder",
    "id": "qwen3-coder-plus",
    "maxOutputTokens": 65536,
    "sourceUrl":
        "https://help.aliyun.com/zh/model-studio/text-generation-model/",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 262144,
    "description": "面向多文件工程、调试和高吞吐 Agent 工作流的 Qwen 代码模型。",
    "displayName": "Qwen 3 Coder Next",
    "family": "Qwen 3 Coder",
    "id": "qwen3-coder-next",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://huggingface.co/Qwen/Qwen3-Coder-Next",
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 262144,
    "description": "Qwen 大规模开源代码模型，针对 Agentic Coding、工具使用和环境交互优化。",
    "displayName": "Qwen 3 Coder 480B A35B Instruct",
    "family": "Qwen 3 Coder",
    "id": "qwen3-coder-480b-a35b-instruct",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://huggingface.co/Qwen/Qwen3-Coder-480B-A35B-Instruct",
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "支持超长上下文、中文和多语言任务的通义千问增强模型。",
    "displayName": "Qwen Plus",
    "family": "Qwen",
    "id": "qwen-plus",
    "maxOutputTokens": 32768,
    "sourceUrl": "https://help.aliyun.com/zh/model-studio/models",
  },
  {
    "contextWindowTokens": 131072,
    "description": "通义千问旗舰型号，适合复杂推理、写作和长文本任务。",
    "displayName": "Qwen Max",
    "family": "Qwen",
    "id": "qwen-max",
    "maxOutputTokens": 8192,
    "sourceUrl": "https://help.aliyun.com/zh/model-studio/models",
  },
  {
    "contextWindowTokens": 131072,
    "description": "通义千问低成本通用模型，支持思考模式、函数调用、内置工具和结构化输出。",
    "displayName": "Qwen Turbo",
    "family": "Qwen",
    "id": "qwen-turbo",
    "maxOutputTokens": 8192,
    "sourceUrl":
        "https://help.aliyun.com/zh/model-studio/text-generation-model/",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "Kimi 原生多模态旗舰模型，面向长程编程、知识工作、推理和 Agent 任务。",
    "displayName": "Kimi K3",
    "family": "Kimi K3",
    "id": "kimi-k3",
    "maxOutputTokens": 1048576,
    "sourceUrl": "https://platform.kimi.com/docs/guide/kimi-k3-quickstart",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": ["kimi-k2.7-code-highspeed"],
    "contextWindowTokens": 262144,
    "description": "Moonshot 面向长程软件工程任务的多模态代码 Agent 模型。",
    "displayName": "Kimi K2.7 Code",
    "family": "Kimi K2",
    "id": "kimi-k2.7-code",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://platform.kimi.com/docs/models",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": ["kimi-k2-250905", "kimi-k2-instruct-0905"],
    "contextWindowTokens": 262144,
    "description":
        "Kimi K2 Instruct 的 0905 增强快照，将上下文扩展至 256K，并强化编码和 Agentic 工具调用。",
    "displayName": "Kimi K2 Instruct 0905",
    "family": "Kimi K2",
    "id": "kimi-k2-0905-preview",
    "sourceUrl": "https://huggingface.co/moonshotai/Kimi-K2-Instruct-0905",
    "supportsToolCalling": true,
  },
  {
    "aliases": ["kimi-k2-250711", "kimi-k2-instruct"],
    "contextWindowTokens": 131072,
    "description": "Kimi K2 首个开源 Instruct 快照，面向代码、通用 Agent 和工具调用任务。",
    "displayName": "Kimi K2 Instruct 0711",
    "family": "Kimi K2",
    "id": "kimi-k2-0711-preview",
    "sourceUrl": "https://huggingface.co/moonshotai/Kimi-K2-Instruct",
    "supportsToolCalling": true,
  },
  {
    "aliases": ["kimi-k2-thinking-251104"],
    "contextWindowTokens": 262144,
    "description": "面向深度推理与通用 Agent 的 Kimi 思考模型，擅长通过多步工具调用解决复杂问题。",
    "displayName": "Kimi K2 Thinking",
    "family": "Kimi K2",
    "id": "kimi-k2-thinking",
    "maxOutputTokens": 65536,
    "sourceUrl": "https://platform.kimi.com/docs/pricing/chat",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 262144,
    "description": "Kimi 原生多模态推理模型，支持图像、文本、思考与非思考模式以及对话和 Agent 任务。",
    "displayName": "Kimi K2.5",
    "family": "Kimi K2",
    "id": "kimi-k2.5",
    "maxOutputTokens": 32768,
    "sourceUrl": "https://platform.kimi.com/docs/pricing/chat",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 262144,
    "description": "Kimi 长任务旗舰模型，强化编码、指令遵循与自我纠错，并支持文本、图像和视频输入。",
    "displayName": "Kimi K2.6",
    "family": "Kimi K2",
    "id": "kimi-k2.6",
    "maxOutputTokens": 32768,
    "sourceUrl": "https://platform.kimi.com/docs/pricing/chat",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 8192,
    "description": "Moonshot 短上下文型号，适合快速对话和短文本生成。",
    "displayName": "Moonshot V1 8K",
    "family": "Kimi",
    "id": "moonshot-v1-8k",
    "maxOutputTokens": 4096,
    "sourceUrl": "https://platform.moonshot.cn/docs/guide/start-using-kimi-api",
  },
  {
    "contextWindowTokens": 32768,
    "description": "Moonshot 中等上下文型号，适合报告和长文档问答。",
    "displayName": "Moonshot V1 32K",
    "family": "Kimi",
    "id": "moonshot-v1-32k",
    "maxOutputTokens": 8192,
    "sourceUrl": "https://platform.moonshot.cn/docs/guide/start-using-kimi-api",
  },
  {
    "contextWindowTokens": 131072,
    "description": "Moonshot 长上下文型号，适合多文档分析和研究任务。",
    "displayName": "Moonshot V1 128K",
    "family": "Kimi",
    "id": "moonshot-v1-128k",
    "maxOutputTokens": 8192,
    "sourceUrl": "https://platform.moonshot.cn/docs/guide/start-using-kimi-api",
  },
  {
    "contextWindowTokens": 1000000,
    "description": "小米旗舰长上下文模型，强化复杂软件工程、通用 Agent 与千次以上工具调用的长程任务。",
    "displayName": "MiMo-V2.5 Pro",
    "family": "MiMo",
    "id": "mimo-v2.5-pro",
    "knowledgeCutoff": "2024-12",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://platform.xiaomimimo.com/",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 131072,
    "description": "OpenAI 开放权重推理模型，面向高性能 Agent、函数调用和结构化输出。",
    "displayName": "gpt-oss-120b",
    "family": "gpt-oss",
    "id": "gpt-oss-120b",
    "knowledgeCutoff": "2024-06",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-oss-120b",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 131072,
    "description": "OpenAI 中型开放权重推理模型，适合低延迟、本地和专业化 Agent 场景。",
    "displayName": "gpt-oss-20b",
    "family": "gpt-oss",
    "id": "gpt-oss-20b",
    "knowledgeCutoff": "2024-06",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://developers.openai.com/api/docs/models/gpt-oss-20b",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "MiniMax 多模态编码与代理模型，提供百万级上下文。",
    "displayName": "MiniMax M3",
    "family": "MiniMax",
    "id": "minimax-m3",
    "maxOutputTokens": 524288,
    "sourceUrl": "https://platform.minimaxi.com/docs/guides/models-intro",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": ["minimax-m2.7-highspeed"],
    "contextWindowTokens": 204800,
    "description": "MiniMax 自进化编码与 Agent 模型，面向复杂任务，并提供更高速的同能力版本。",
    "displayName": "MiniMax M2.7",
    "family": "MiniMax M2",
    "id": "minimax-m2.7",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://platform.minimaxi.com/docs/guides/models-intro",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "aliases": ["minimax-m2.5-highspeed", "minimax-m2.5-free"],
    "contextWindowTokens": 204800,
    "description": "兼顾复杂任务能力与成本的 MiniMax 编码和 Agent 模型，并提供高速推理版本。",
    "displayName": "MiniMax M2.5",
    "family": "MiniMax M2",
    "id": "minimax-m2.5",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://platform.minimaxi.com/docs/guides/models-intro",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "aliases": ["minimax-m2.1-highspeed"],
    "contextWindowTokens": 204800,
    "description": "强化多语言编程能力的 MiniMax 模型，适合代码生成、工程任务和 Agent 工作流。",
    "displayName": "MiniMax M2.1",
    "family": "MiniMax M2",
    "id": "minimax-m2.1",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://platform.minimaxi.com/docs/guides/models-intro",
  },
  {
    "aliases": ["doubao-seed-2-1-pro", "doubao-seed-2-1-pro-260628"],
    "contextWindowTokens": 256000,
    "description": "字节跳动新一代 Agent 通用旗舰模型，强化编码、代理规划、长链路执行和原生多模态理解。",
    "displayName": "Doubao Seed 2.1 Pro",
    "family": "Doubao Seed 2.1",
    "id": "doubao-seed-2.1-pro",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": ["doubao-seed-2-1-turbo", "doubao-seed-2-1-turbo-260628"],
    "contextWindowTokens": 256000,
    "description": "面向高并发生产场景的豆包模型，在推理、Agent 能力、速度和成本之间取得平衡。",
    "displayName": "Doubao Seed 2.1 Turbo",
    "family": "Doubao Seed 2.1",
    "id": "doubao-seed-2.1-turbo",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": ["doubao-seed-2-0-pro", "doubao-seed-2-0-pro-260215"],
    "contextWindowTokens": 256000,
    "description": "字节跳动旗舰 Agent 通用模型，重点提升复杂任务规划、推理和执行能力。",
    "displayName": "Doubao Seed 2.0 Pro",
    "family": "Doubao Seed 2.0",
    "id": "doubao-seed-2.0-pro",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": [
      "doubao-seed-2-0-lite",
      "doubao-seed-2-0-lite-260215",
      "doubao-seed-2-0-lite-260428",
    ],
    "contextWindowTokens": 256000,
    "description": "兼顾成本和通用任务能力的豆包多模态深度推理模型，适合常规生产工作负载。",
    "displayName": "Doubao Seed 2.0 Lite",
    "family": "Doubao Seed 2.0",
    "id": "doubao-seed-2.0-lite",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": [
      "doubao-seed-2-0-mini",
      "doubao-seed-2-0-mini-260215",
      "doubao-seed-2-0-mini-260428",
    ],
    "contextWindowTokens": 256000,
    "description": "面向轻量任务和高并发场景的豆包快速模型，保留多模态、推理和工具调用能力。",
    "displayName": "Doubao Seed 2.0 Mini",
    "family": "Doubao Seed 2.0",
    "id": "doubao-seed-2.0-mini",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": ["doubao-seed-2-0-code", "doubao-seed-2-0-code-preview-260215"],
    "contextWindowTokens": 256000,
    "description": "针对 Agentic Coding 深度优化的豆包模型，支持多模态输入、代码任务和工具工作流。",
    "displayName": "Doubao Seed 2.0 Code",
    "family": "Doubao Seed 2.0",
    "id": "doubao-seed-2.0-code",
    "maxOutputTokens": 128000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": ["doubao-seed-1-8", "doubao-seed-1-8-251228"],
    "contextWindowTokens": 256000,
    "description": "强化多模态理解与 Agent 能力的豆包模型，支持文本、图像、视频和上下文缓存。",
    "displayName": "Doubao Seed 1.8",
    "family": "Doubao Seed 1.8",
    "id": "doubao-seed-1.8",
    "maxOutputTokens": 64000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": ["doubao-seed-code-preview", "doubao-seed-code-preview-251028"],
    "contextWindowTokens": 256000,
    "description": "豆包代码和 Agent 工作流模型，支持文本、图像、视频输入及 Anthropic API 兼容调用。",
    "displayName": "Doubao Seed Code",
    "family": "Doubao Seed",
    "id": "doubao-seed-code",
    "maxOutputTokens": 32000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": [
      "doubao-seed-1-6",
      "doubao-seed-1-6-250615",
      "doubao-seed-1-6-251015",
    ],
    "contextWindowTokens": 256000,
    "description": "豆包多模态深度推理模型，提供自动、思考和非思考模式，适合通用对话与 Agent 任务。",
    "displayName": "Doubao Seed 1.6",
    "family": "Doubao Seed 1.6",
    "id": "doubao-seed-1.6",
    "maxOutputTokens": 32000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": [
      "doubao-seed-1-6-flash",
      "doubao-seed-1-6-flash-250615",
      "doubao-seed-1-6-flash-250828",
    ],
    "contextWindowTokens": 256000,
    "description": "低延迟的豆包多模态深度推理模型，面向快速文本、视觉理解和高吞吐任务。",
    "displayName": "Doubao Seed 1.6 Flash",
    "family": "Doubao Seed 1.6",
    "id": "doubao-seed-1.6-flash",
    "maxOutputTokens": 32000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": ["doubao-seed-1-6-vision", "doubao-seed-1-6-vision-250815"],
    "contextWindowTokens": 256000,
    "description": "针对图像、文档、教育和检索问答优化的豆包视觉深度推理模型。",
    "displayName": "Doubao Seed 1.6 Vision",
    "family": "Doubao Seed 1.6",
    "id": "doubao-seed-1.6-vision",
    "maxOutputTokens": 32000,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": ["doubao-1-5-pro-32k", "doubao-1-5-pro-32k-250115"],
    "contextWindowTokens": 128000,
    "description": "豆包 1.5 旗舰通用模型，全面提升知识、编码和推理能力。",
    "displayName": "Doubao 1.5 Pro 32K",
    "family": "Doubao 1.5",
    "id": "doubao-1.5-pro-32k",
    "maxOutputTokens": 16384,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": ["doubao-1-5-pro-256k", "doubao-1-5-pro-256k-250115"],
    "contextWindowTokens": 256000,
    "description": "豆包 1.5 Pro 的长上下文版本，适合大文档、复杂对话和长输入任务。",
    "displayName": "Doubao 1.5 Pro 256K",
    "family": "Doubao 1.5",
    "id": "doubao-1.5-pro-256k",
    "maxOutputTokens": 12288,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": ["doubao-1-5-lite-32k", "doubao-1-5-lite-32k-250115"],
    "contextWindowTokens": 32768,
    "description": "面向低延迟和高性价比场景的豆包 1.5 轻量通用模型。",
    "displayName": "Doubao 1.5 Lite 32K",
    "family": "Doubao 1.5",
    "id": "doubao-1.5-lite-32k",
    "maxOutputTokens": 12288,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
  },
  {
    "aliases": [
      "doubao-1.5-vision-pro",
      "doubao-1-5-vision-pro-32k",
      "doubao-1-5-vision-pro-32k-250115",
    ],
    "contextWindowTokens": 32768,
    "description": "豆包 1.5 多模态旗舰模型，强化视觉推理、文档识别、细节理解和指令遵循。",
    "displayName": "Doubao 1.5 Vision Pro 32K",
    "family": "Doubao 1.5",
    "id": "doubao-1.5-vision-pro-32k",
    "maxOutputTokens": 12288,
    "sourceUrl": "https://www.volcengine.com/docs/82379/1330310",
    "supportsVision": true,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "智谱旗舰文本推理模型，强化复杂软件工程、长程任务和工具调用。",
    "displayName": "GLM-5.3",
    "family": "GLM-5",
    "id": "glm-5.3",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/models/text/glm-5.3",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVision": false,
  },
  {
    "contextWindowTokens": 1000000,
    "description": "智谱高效原生多模态模型，支持图像与视频理解、视觉编码和专业文档任务。",
    "displayName": "GLM-5.3 Flash",
    "family": "GLM-5",
    "id": "glm-5.3-flash",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/models/vlm/glm-5.3-flash",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": ["glm-5-2-260617"],
    "contextWindowTokens": 1000000,
    "description": "智谱面向长程任务的旗舰模型，提供百万级上下文，并强化长期规划、复杂编码和 Agent 执行。",
    "displayName": "GLM-5.2",
    "family": "GLM-5",
    "id": "glm-5.2",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/start/model-overview",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 200000,
    "description": "智谱长程任务旗舰模型，强化复杂编码、自主规划、持续执行和迭代能力。",
    "displayName": "GLM-5.1",
    "family": "GLM-5",
    "id": "glm-5.1",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/start/model-overview",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "aliases": ["glm-5-turbo"],
    "contextWindowTokens": 200000,
    "description": "面向 Agentic Engineering 的智谱旗舰模型，擅长复杂系统工程、编码和长程代理任务。",
    "displayName": "GLM-5",
    "family": "GLM-5",
    "id": "glm-5",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/start/model-overview",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "aliases": ["glm-4-7", "zai-glm-4.7"],
    "contextWindowTokens": 200000,
    "description": "智谱 Agentic Coding 旗舰模型，强化编码、长任务规划、工具协作和复杂指令遵循。",
    "displayName": "GLM-4.7",
    "family": "GLM-4",
    "id": "glm-4.7",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/start/model-overview",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "aliases": ["glm-4.7-flashx"],
    "contextWindowTokens": 200000,
    "description": "智谱 30B 级 Agentic Coding 模型，兼顾效率、长任务规划和工具协作。",
    "displayName": "GLM-4.7 Flash",
    "family": "GLM-4",
    "id": "glm-4.7-flash",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/start/model-overview",
    "supportsReasoning": true,
    "supportsToolCalling": true,
  },
  {
    "contextWindowTokens": 200000,
    "description": "智谱通用旗舰模型，重点提升高级编码、长文本处理、推理和 Agent 能力。",
    "displayName": "GLM-4.6",
    "family": "GLM-4",
    "id": "glm-4.6",
    "maxOutputTokens": 131072,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/start/model-overview",
  },
  {
    "contextWindowTokens": 131072,
    "description": "智谱原生多模态视觉推理模型，支持图像、视频、文件、深度思考和工具调用。",
    "displayName": "GLM-4.6V",
    "family": "GLM-4",
    "id": "glm-4.6v",
    "maxOutputTokens": 32768,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/models/vlm/glm-4.6v",
    "supportsReasoning": true,
    "supportsToolCalling": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": ["glm-4.5-air", "glm-4.5-airx", "glm-4.5-x"],
    "contextWindowTokens": 131072,
    "description": "智谱面向 Agent 的混合推理模型，支持思考与非思考模式，兼顾推理、编码和工具调用。",
    "displayName": "GLM-4.5",
    "family": "GLM-4",
    "id": "glm-4.5",
    "maxOutputTokens": 98304,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/start/model-overview",
  },
  {
    "contextWindowTokens": 65536,
    "description": "智谱 MoE 视觉推理模型，覆盖图像、视频、文档理解和 GUI Agent，并支持思考模式。",
    "displayName": "GLM-4.5V",
    "family": "GLM-4",
    "id": "glm-4.5v",
    "maxOutputTokens": 16384,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/models/vlm/glm-4.5v",
    "supportsReasoning": true,
    "supportsVideo": true,
    "supportsVision": true,
  },
  {
    "aliases": ["glm-4-flash"],
    "contextWindowTokens": 131072,
    "description": "智谱长文本通用模型，适合复杂任务和中文内容处理。",
    "displayName": "GLM-4 Plus",
    "family": "GLM-4",
    "id": "glm-4-plus",
    "maxOutputTokens": 4096,
    "sourceUrl": "https://docs.bigmodel.cn/cn/guide/models/text/glm-4",
  },
  {
    "contextWindowTokens": 1000000,
    "description": "Anthropic 面向高难度代理、编码和专业任务的高级推理模型。",
    "displayName": "Claude Opus 4.6",
    "family": "Claude 4.6",
    "id": "claude-opus-4-6",
    "knowledgeCutoff": "2025-05",
    "maxOutputTokens": 128000,
    "sourceUrl":
        "https://docs.anthropic.com/en/docs/about-claude/models/overview",
  },
  {
    "contextWindowTokens": 1000000,
    "description": "Anthropic 面向编码、代理与复杂推理的长上下文模型。",
    "displayName": "Claude Sonnet 4.6",
    "family": "Claude 4.6",
    "id": "claude-sonnet-4-6",
    "knowledgeCutoff": "2025-08",
    "maxOutputTokens": 64000,
    "sourceUrl":
        "https://docs.anthropic.com/en/docs/about-claude/models/overview",
  },
  {
    "aliases": ["claude-opus-4-5", "claude-haiku-4-5"],
    "contextWindowTokens": 200000,
    "description": "Anthropic 通用长上下文模型，支持编码、视觉与扩展思考。",
    "displayName": "Claude 4.5",
    "family": "Claude 4.5",
    "id": "claude-sonnet-4-5",
    "maxOutputTokens": 64000,
    "sourceUrl":
        "https://docs.anthropic.com/en/docs/about-claude/models/overview",
  },
  {
    "aliases": ["grok-4.20", "grok-4.20-0309-non-reasoning"],
    "contextWindowTokens": 1000000,
    "description": "xAI 长上下文通用模型，支持工具调用、视觉和结构化输出。",
    "displayName": "Grok 4",
    "family": "Grok",
    "id": "grok-4.3",
    "sourceUrl": "https://docs.x.ai/docs/models",
  },
];
const imageGenerationModelIds = [
  "gpt-image-2",
  "gpt-image-2.5-sunburst",
  "gpt-image-2.5-flare",
];
